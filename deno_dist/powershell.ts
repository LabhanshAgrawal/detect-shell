...
function getProgramFilesPath({useAlternateBitness = false}: {useAlternateBitness?: boolean} = {}):
  | string
  | null {
...
async function findPSCoreMsix({
  ...
  const msixAppDir = join(env.LOCALAPPDATA, 'Microsoft', 'WindowsApps');
...
function findWinPS({
  useAlternateBitness = false
}: {useAlternateBitness?: boolean} = {}): IPossiblePowerShellExe | null {
  ...
  const winPSPath = join(systemFolderPath, 'WindowsPowerShell', 'v1.0', 'powershell.exe');
